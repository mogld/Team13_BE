...
public class ExcelUploadService {
    ...
    private void processRecipientRow(Row row, List<ExcelRecipientResponseDto> successList,
        ...
        String institution = getCellValue(row.getCell(6));
        ...
        try {
            ...
            successList.add(new ExcelRecipientResponseDto(
                    recipient.getId(), name, LocalDate.parse(birth), gender, careLevel, careNumber, LocalDate.parse(startDate), institution));
        } catch (ApplicationException e) {
            failedList.add(new ExcelRecipientResponseDto(
                    null, name, LocalDate.parse(birth), gender, careLevel, careNumber, LocalDate.parse(startDate), institution));
        }
    }
